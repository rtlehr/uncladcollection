...
<PageHeader title="Campaign Performance Analytics" description="Measure campaign reach, engagement, and marketplace influence.">
<template #actions><div class="flex gap-2"><Button variant="outline" as-child><Link href="/admin/analytics">Marketplace intelligence</Link></Button><Button variant="outline" as-child><a :href="exportUrl()">Export CSV</a></Button></div></template>
</PageHeader>
<form class="grid gap-4 rounded-xl border bg-background p-4 xl:grid-cols-[1.4fr_1fr_1fr_1fr_auto] xl:items-end" @submit.prevent="apply">
