...
async function generate() {
    ...
    try {
        ...
        if (!response.ok) throw new Error(payload.message ?? 'Prompt generation failed.');
        ...
        if (!form.title.trim()) form.title = form.description.slice(0, 120);
    } catch (e) { generationError.value = e instanceof Error ? e.message : 'Prompt generation failed.'; }
    finally { generating.value = false; }
}

function save() { form.post('/admin/ai-content/image-prompts'); }
</script>
