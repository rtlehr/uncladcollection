...
<template><Head :title="ticket.ticket_number"/><AccountPageLayout><template #title>My Tickets</template><template #description>Review this request and continue the conversation.</template><div class="mx-auto max-w-4xl space-y-6"><PageHeader :title="ticket.subject" :description="`${ticket.ticket_number} · ${ticket.category || 'General'}`"><template #actions><StatusBadge :status="ticket.status"/></template></PageHeader>
