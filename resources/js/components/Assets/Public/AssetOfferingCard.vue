...
const props = defineProps<{ offering: PublicAssetOffering; purchaseUrl: string | null }>();
...
<template>
    <article class="flex h-full flex-col rounded-3xl border border-stone-200 bg-white p-6 shadow-sm dark:border-stone-800 dark:bg-stone-900">
        <div class="flex items-start justify-between gap-4"><div><p class="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--brand-accent)]">{{ offering.license_type.name }}</p><h3 class="mt-2 text-xl font-semibold">{{ offering.name }}</h3></div><p class="text-xl font-bold">{{ money(offering.price_cents, offering.currency) }}</p></div>
