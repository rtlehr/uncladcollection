...
class PublicPageTransferService
{
    ...
    {
        $entries=PublicPage::with('faqItems')->orderBy('sort_order')->orderBy('title')->get()->map(fn(PublicPage $p)=>[
            'title'=>$p->title,'slug'=>$p->slug,'eyebrow'=>$p->eyebrow,'introduction'=>$p->introduction,'content'=>$p->content,
    ...
    public function decode(string $json): array
    {
        ...
        $v=Validator::make($payload??[],['format'=>['required',Rule::in([self::FORMAT])],'version'=>['required','integer',Rule::in([self::VERSION])],'entries'=>['required','array'],'entries.*.title'=>['required','string','max:255'],'entries.*.slug'=>['required','alpha_dash','max:255'],'entries.*.page_type'=>['required',Rule::in(array_keys(config('public-pages.types',[])))],'entries.*.status'=>['required',Rule::in(['draft','published'])],'entries.*.is_active'=>['required','boolean'],'entries.*.faq_items'=>['present','array']]);
    ...
    public function importJson(string $json,string $mode='merge',bool $dryRun=false): array
    {
        ...
        $work=function()use($payload,$mode,$dryRun,&$summary){$slugs=[]; foreach($payload['entries'] as $entry){$slugs[]=$entry['slug'];$existing=PublicPage::with('faqItems')->where('slug',$entry['slug'])->first();$attrs=Arr::except($entry,['faq_items']);$attrs['published_at']=$entry['published_at']?Carbon::parse($entry['published_at']):null;$changed=!$existing||collect($attrs)->contains(fn($v,$k)=>$existing->{$k}!=$v)||($existing?->faqItems->map->only(['question','answer','is_active','sort_order'])->values()->all()!==$entry['faq_items']);$summary[$existing?($changed?'updated':'unchanged'):'created']++;if($dryRun)continue;$page=PublicPage::updateOrCreate(['slug'=>$entry['slug']],$attrs);$page->faqItems()->delete();foreach($entry['faq_items'] as $faq)$page->faqItems()->create($faq);}if($mode==='replace'){$q=PublicPage::whereNotIn('slug',$slugs);$summary['deleted']=$q->count();if(!$dryRun)$q->delete();}};
