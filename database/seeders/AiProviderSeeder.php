...
class AiProviderSeeder extends Seeder
{
    public function run(): void
    {
        $ollama = AiProvider::query()->firstOrCreate(['slug' => 'ollama'], [
            ...
            'driver' => 'ollama',
            'base_url' => (string) config('ai-assets.providers.ollama.base_url', 'https://ai.uncladcollection.com'),
            'api_key' => (string) config('ai-assets.providers.ollama.token'),
            'default_model' => (string) config('ai-assets.providers.ollama.model', 'qwen3-vl:8b'),
        ...
        AiProvider::query()->firstOrCreate(['slug' => 'venice'], [
            'name' => 'Venice AI', 'driver' => 'venice', 'base_url' => 'https://api.venice.ai/api/v1',
            'default_model' => null, 'connect_timeout_seconds' => 20, 'timeout_seconds' => 300,
            'retry_times' => 2, 'is_enabled' => false,
        ]);

        AiProvider::query()->firstOrCreate(['slug' => 'openai'], [
            'name' => 'OpenAI', 'driver' => 'openai', 'base_url' => 'https://api.openai.com/v1',
            'default_model' => 'gpt-4.1-mini', 'connect_timeout_seconds' => 20, 'timeout_seconds' => 300,
            'retry_times' => 2, 'is_enabled' => false,
        ]);

        foreach (array_keys(AiFeatureCatalog::FEATURES) as $feature) {
            AiFeatureAssignment::query()->firstOrCreate(['feature' => $feature], [
