...
Route::prefix('support')->name('support.')->group(function (): void {
    ...
    Route::post('/', [GuestSupportController::class, 'store'])->middleware('throttle:6,1')->name('store');

    Route::get('/guest/{ticket}/{token}', [GuestSupportController::class, 'show'])->name('guest.show');
    Route::post('/guest/{ticket}/{token}/reply', [GuestSupportController::class, 'reply'])->middleware('throttle:12,1')->name('guest.reply');
    ...
    Route::middleware(['auth', 'verified'])->group(function (): void {
        ...
        Route::post('/tickets', [MemberSupportController::class, 'store'])->name('member.store');
        Route::get('/tickets/{ticket}', [MemberSupportController::class, 'show'])->name('show');
        Route::post('/tickets/{ticket}/reply', [MemberSupportController::class, 'reply'])->name('reply');
        Route::post('/tickets/{ticket}/close', [MemberSupportController::class, 'close'])->name('close');
        Route::post('/tickets/{ticket}/reopen', [MemberSupportController::class, 'reopen'])->name('reopen');
